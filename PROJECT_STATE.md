...
**▶ RESUME HERE:** **Manual QA + Mac build.** (1) Runtime behaviors that can't be auto-tested (see checklist below): mic → dictation, hotkey toggle, drag/persist, context menu, settings-preserve-dictionary, history playback, auto-paste into field. (2) **Mac build**: clone the repo on a Mac, `npm run tauri build -- --features metal` (prereqs in BUILD.md). (3) **Auto-paste is Windows-only** (`SendInput`) — add a macOS path (e.g. `enigo` crate) as a follow-up. (4) Deferred: wire `mic_device` to cpal device selection.
