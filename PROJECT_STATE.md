...
**Published (2026-09-09):** confirmed working by Dave. Repo is now a **git repo pushed to GitHub → https://github.com/kamicrafted/voxable** (private, branch `main`). **Release `v0.1.0`** cut with the Windows installers attached (CPU setup.exe + msi, shareable CUDA setup.exe). README rewritten with per-platform install + Flow Bar/Hub screenshots (`docs/screenshots/`). `.gitignore`/`.gitattributes` keep `target/`, `node_modules/`, `dist/`, `src-tauri/cuda-runtime/`, and installers out of the repo. Auth: `gh` (account `kamicrafted`) + HTTPS remote via `gh auth setup-git`.
...
**▶ RESUME HERE:** **Manual QA on the Mac, permissions first.** (1) Run the onboarding screen end to end: allow Microphone, allow Accessibility, confirm the Fn listener starts (log line "Fn key listener running" in `~/Library/Logs/Voxable/voxable.log`). (2) Then the runtime checklist below: mic → dictation, `fn` toggle, drag/persist, context menu, settings-preserve-dictionary, history playback, auto-paste into a field. (3) Port the onboarding screen to Windows (it currently reports every permission as granted there). (4) Deferred: wire `mic_device` to cpal device selection.
