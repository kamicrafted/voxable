...
**▶ RESUME HERE:** **Manual QA + macOS auto-paste.** (1) Runtime behaviors that can't be auto-tested (see checklist below): mic → dictation, hotkey toggle, drag/persist, context menu, settings-preserve-dictionary, history playback, auto-paste into field — now on Mac as well as Windows. (2) **Auto-paste and foreground-app detection are Windows-only** (`SendInput` / Win32); `app_context.rs` stubs them out on macOS, so dictation gets you clipboard copy but no auto-type. Add a macOS path (e.g. `enigo`). (3) Deferred: wire `mic_device` to cpal device selection.
